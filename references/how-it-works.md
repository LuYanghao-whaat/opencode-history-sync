...
- 每台机器把「本机新事件」写成**一包一包的加密文件**，push 到私有仓库；换机器时 pull 下来。
- 文件名形如 `bundles/<UTC时间戳>__<机器名>__<会话id>__<最小seq>__<最大seq>.jsonl.enc`。文件名即索引：判断某个会话有没有新包、仓库各会话进度到哪，都无需解密整包。
- 文件名唯一，天然**无合并冲突**；并发 push 只需 `pull --rebase` 重试。
- 因为托管服务总在线，**任意一台随时可同步**，不需要任何设备常开、不需要同一局域网。
